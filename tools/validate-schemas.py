...
Dependencies: the `jsonschema` package (`pip install jsonschema`). If the
package is missing the script degrades gracefully to a structural check
(parse-only) and emits a warning. Full validation in CI requires the
dependency.
"""
...
def validate_one(schema: dict, artifact_path: pathlib.Path, jsonschema_mod) -> ValidationReport:
